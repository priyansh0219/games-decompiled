...
namespace UnityStandardAssets.ImageEffects
{
	...
	public class NoiseAndGrain : PostEffectsBase
	{
		...
		private void OnRenderImage(RenderTexture source, RenderTexture destination)
		{
			...
			if (dx11Grain && supportDX11)
			{
				dx11NoiseMaterial.SetFloat(ShaderPropertyID._DX11NoiseTime, Time.frameCount);
				dx11NoiseMaterial.SetTexture(ShaderPropertyID._NoiseTex, noiseTexture);
				dx11NoiseMaterial.SetVector(ShaderPropertyID._NoisePerChannel, monochrome ? Vector3.one : intensities);
				dx11NoiseMaterial.SetVector(ShaderPropertyID._MidGrey, new Vector3(midGrey, 1f / (1f - midGrey), -1f / midGrey));
				dx11NoiseMaterial.SetVector(ShaderPropertyID._NoiseAmount, new Vector3(generalIntensity, blackIntensity, whiteIntensity) * intensityMultiplier);
				if (softness > Mathf.Epsilon)
				{
					...
					dx11NoiseMaterial.SetTexture(ShaderPropertyID._NoiseTex, temporary);
			...
			noiseMaterial.SetTexture(ShaderPropertyID._NoiseTex, noiseTexture);
			noiseMaterial.SetVector(ShaderPropertyID._NoisePerChannel, monochrome ? Vector3.one : intensities);
			noiseMaterial.SetVector(ShaderPropertyID._NoiseTilingPerChannel, monochrome ? (Vector3.one * monochromeTiling) : tiling);
			noiseMaterial.SetVector(ShaderPropertyID._MidGrey, new Vector3(midGrey, 1f / (1f - midGrey), -1f / midGrey));
			noiseMaterial.SetVector(ShaderPropertyID._NoiseAmount, new Vector3(generalIntensity, blackIntensity, whiteIntensity) * intensityMultiplier);
			if (softness > Mathf.Epsilon)
			{
				...
				noiseMaterial.SetTexture(ShaderPropertyID._NoiseTex, temporary2);
		...
		private static void DrawNoiseQuadGrid(RenderTexture source, RenderTexture dest, Material fxMaterial, Texture2D noise, int passNr)
		{
			...
			fxMaterial.SetTexture(ShaderPropertyID._MainTex, source);
			GL.PushMatrix();
			GL.LoadOrtho();
