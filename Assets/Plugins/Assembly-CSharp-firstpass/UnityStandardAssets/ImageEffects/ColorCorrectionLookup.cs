...
namespace UnityStandardAssets.ImageEffects
{
	...
	public class ColorCorrectionLookup : PostEffectsBase
	{
		...
		private void OnRenderImage(RenderTexture source, RenderTexture destination)
		{
			...
			material.SetFloat(ShaderPropertyID._Scale, (float)(width - 1) / (1f * (float)width));
			material.SetFloat(ShaderPropertyID._Offset, 1f / (2f * (float)width));
			material.SetTexture(ShaderPropertyID._ClutTex, converted3DLut);
