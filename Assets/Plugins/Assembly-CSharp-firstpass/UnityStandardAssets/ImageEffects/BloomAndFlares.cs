	...
	public class BloomAndFlares : PostEffectsBase
	{
		...
		private void OnRenderImage(RenderTexture source, RenderTexture destination)
		{
			...
			if (lensflares)
			{
				...
				{
					hollywoodFlaresMaterial.SetVector(ShaderPropertyID._threshold, new Vector4(lensflareThreshold, 1f / (1f - lensflareThreshold), 0f, 0f));
					hollywoodFlaresMaterial.SetVector(ShaderPropertyID.tintColor, new Vector4(flareColorA.r, flareColorA.g, flareColorA.b, flareColorA.a) * flareColorA.a * lensflareIntensity);
					...
					temporary3.DiscardContents();
					hollywoodFlaresMaterial.SetVector(ShaderPropertyID.offsets, new Vector4(sepBlurSpread * 1f / num * num2, 0f, 0f, 0f));
					hollywoodFlaresMaterial.SetFloat(ShaderPropertyID.stretchWidth, hollyStretchWidth);
					Graphics.Blit(temporary4, temporary3, hollywoodFlaresMaterial, 1);
					temporary4.DiscardContents();
					hollywoodFlaresMaterial.SetFloat(ShaderPropertyID.stretchWidth, hollyStretchWidth * 2f);
					Graphics.Blit(temporary3, temporary4, hollywoodFlaresMaterial, 1);
					temporary3.DiscardContents();
					hollywoodFlaresMaterial.SetFloat(ShaderPropertyID.stretchWidth, hollyStretchWidth * 4f);
					Graphics.Blit(temporary4, temporary3, hollywoodFlaresMaterial, 1);
					temporary4.DiscardContents();
					if (lensflareMode == LensflareStyle34.Anamorphic)
					{
						...
						{
							separableBlurMaterial.SetVector(ShaderPropertyID.offsets, new Vector4(hollyStretchWidth * 2f / num * num2, 0f, 0f, 0f));
			...
			screenBlend.SetFloat(ShaderPropertyID._Intensity, bloomIntensity);
			screenBlend.SetTexture(ShaderPropertyID._ColorBuffer, source);
		...
		{
			addBrightStuffBlendOneOneMaterial.SetFloat(ShaderPropertyID._Intensity, intensity_);
			Graphics.Blit(from, to, addBrightStuffBlendOneOneMaterial);
		}

		private void BlendFlares(RenderTexture from, RenderTexture to)
		{
			lensFlareMaterial.SetVector(ShaderPropertyID.colorA, new Vector4(flareColorA.r, flareColorA.g, flareColorA.b, flareColorA.a) * lensflareIntensity);
			lensFlareMaterial.SetVector(ShaderPropertyID.colorB, new Vector4(flareColorB.r, flareColorB.g, flareColorB.b, flareColorB.a) * lensflareIntensity);
			lensFlareMaterial.SetVector(ShaderPropertyID.colorC, new Vector4(flareColorC.r, flareColorC.g, flareColorC.b, flareColorC.a) * lensflareIntensity);
			lensFlareMaterial.SetVector(ShaderPropertyID.colorD, new Vector4(flareColorD.r, flareColorD.g, flareColorD.b, flareColorD.a) * lensflareIntensity);
		...
		private void BrightFilter(float thresh, float useAlphaAsMask, RenderTexture from, RenderTexture to)
		{
			if (doHdr)
			{
				brightPassFilterMaterial.SetVector(ShaderPropertyID.threshold, new Vector4(thresh, 1f, 0f, 0f));
			}
			else
			{
				brightPassFilterMaterial.SetVector(ShaderPropertyID.threshold, new Vector4(thresh, 1f / (1f - thresh), 0f, 0f));
			}
			brightPassFilterMaterial.SetFloat(ShaderPropertyID.useSrcAlphaAsMask, useAlphaAsMask);
		...
		private void Vignette(float amount, RenderTexture from, RenderTexture to)
		{
			...
			{
				screenBlend.SetTexture(ShaderPropertyID._ColorBuffer, lensFlareVignetteMask);
				Graphics.Blit(from, to, screenBlend, 3);
			}
			else
			{
				vignetteMaterial.SetFloat(ShaderPropertyID.vignetteIntensity, amount);
