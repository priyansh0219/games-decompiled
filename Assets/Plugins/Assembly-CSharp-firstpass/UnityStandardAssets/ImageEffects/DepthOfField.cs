...
namespace UnityStandardAssets.ImageEffects
{
	...
	public class DepthOfField : PostEffectsBase
	{
		...
		{
			dofHdrMaterial.SetTexture(ShaderPropertyID._FgOverlap, null);
			if (nearBlur && fgDilate)
			{
				...
				dofHdrMaterial.SetVector(ShaderPropertyID._Offsets, new Vector4(0f, num, 0f, num));
				...
				dofHdrMaterial.SetVector(ShaderPropertyID._Offsets, new Vector4(num, 0f, 0f, num));
				...
				dofHdrMaterial.SetTexture(ShaderPropertyID._FgOverlap, temporary);
		...
		private void OnRenderImage(RenderTexture source, RenderTexture destination)
		{
			...
			dofHdrMaterial.SetVector(ShaderPropertyID._CurveParams, new Vector4(1f, focalSize, aperture / 10f, focalDistance01));
			...
			else if (blurType == BlurType.DX11 && (bool)dx11bokehMaterial)
			{
				if (highResolution)
				{
					...
					dofHdrMaterial.SetVector(ShaderPropertyID._Offsets, new Vector4(0f, 1.5f, 0f, 1.5f));
					Graphics.Blit(renderTexture3, renderTexture4, dofHdrMaterial, 19);
					dofHdrMaterial.SetVector(ShaderPropertyID._Offsets, new Vector4(1.5f, 0f, 0f, 1.5f));
					Graphics.Blit(renderTexture4, renderTexture3, dofHdrMaterial, 19);
					if (nearBlur)
					{
						Graphics.Blit(source, renderTexture4, dofHdrMaterial, 4);
					}
					dx11bokehMaterial.SetTexture(ShaderPropertyID._BlurredColor, renderTexture3);
					dx11bokehMaterial.SetFloat(ShaderPropertyID._SpawnHeuristic, dx11SpawnHeuristic);
					dx11bokehMaterial.SetVector(ShaderPropertyID._BokehParams, new Vector4(dx11BokehScale, dx11BokehIntensity, Mathf.Clamp(dx11BokehThreshold, 0.005f, 4f), internalBlurWidth));
					dx11bokehMaterial.SetTexture(ShaderPropertyID._FgCocMask, nearBlur ? renderTexture4 : null);
					...
					dofHdrMaterial.SetVector(ShaderPropertyID._Offsets, new Vector4(internalBlurWidth, 0f, 0f, internalBlurWidth));
					Graphics.Blit(renderTexture, source, dofHdrMaterial, 5);
					dofHdrMaterial.SetVector(ShaderPropertyID._Offsets, new Vector4(0f, internalBlurWidth, 0f, internalBlurWidth));
					Graphics.Blit(source, temporary, dofHdrMaterial, 21);
					Graphics.SetRenderTarget(temporary);
					ComputeBuffer.CopyCount(cbPoints, cbDrawArgs, 0);
					dx11bokehMaterial.SetBuffer(ShaderPropertyID.pointBuffer, cbPoints);
					dx11bokehMaterial.SetTexture(ShaderPropertyID._MainTex, dx11BokehTexture);
					dx11bokehMaterial.SetVector(ShaderPropertyID._Screen, new Vector3(1f / (1f * (float)source.width), 1f / (1f * (float)source.height), internalBlurWidth));
				...
				else
				{
					...
					dx11bokehMaterial.SetFloat(ShaderPropertyID._SpawnHeuristic, dx11SpawnHeuristic);
					dx11bokehMaterial.SetVector(ShaderPropertyID._BokehParams, new Vector4(dx11BokehScale, dx11BokehIntensity, Mathf.Clamp(dx11BokehThreshold, 0.005f, 4f), internalBlurWidth));
					dx11bokehMaterial.SetTexture(ShaderPropertyID._FgCocMask, renderTexture5);
					Graphics.SetRandomWriteTarget(1, cbPoints);
					...
					dx11bokehMaterial.SetBuffer(ShaderPropertyID.pointBuffer, cbPoints);
					dx11bokehMaterial.SetTexture(ShaderPropertyID._MainTex, dx11BokehTexture);
					dx11bokehMaterial.SetVector(ShaderPropertyID._Screen, new Vector3(1f / (1f * (float)renderTexture2.width), 1f / (1f * (float)renderTexture2.height), internalBlurWidth));
					dx11bokehMaterial.SetPass(1);
					Graphics.DrawProceduralIndirectNow(MeshTopology.Points, cbDrawArgs);
					dofHdrMaterial.SetTexture(ShaderPropertyID._LowRez, renderTexture2);
					dofHdrMaterial.SetTexture(ShaderPropertyID._FgOverlap, renderTexture5);
					dofHdrMaterial.SetVector(ShaderPropertyID._Offsets, 1f * (float)source.width / (1f * (float)renderTexture2.width) * internalBlurWidth * Vector4.one);
			...
			else
			{
				...
				if (highResolution)
				{
					dofHdrMaterial.SetVector(ShaderPropertyID._Offsets, new Vector4(0f, internalBlurWidth, 0.025f, internalBlurWidth));
					Graphics.Blit(source, destination, dofHdrMaterial, pass);
				}
				else
				{
					dofHdrMaterial.SetVector(ShaderPropertyID._Offsets, new Vector4(0f, internalBlurWidth, 0.1f, internalBlurWidth));
					Graphics.Blit(source, renderTexture, dofHdrMaterial, 6);
					Graphics.Blit(renderTexture, renderTexture2, dofHdrMaterial, pass);
					dofHdrMaterial.SetTexture(ShaderPropertyID._LowRez, renderTexture2);
					dofHdrMaterial.SetTexture(ShaderPropertyID._FgOverlap, null);
					dofHdrMaterial.SetVector(ShaderPropertyID._Offsets, Vector4.one * (1f * (float)source.width / (1f * (float)renderTexture2.width)) * internalBlurWidth);
