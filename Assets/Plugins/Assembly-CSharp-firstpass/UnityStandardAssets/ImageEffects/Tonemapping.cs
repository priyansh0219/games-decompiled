	...
	public class Tonemapping : PostEffectsBase
	{
		...
		private void OnRenderImage(RenderTexture source, RenderTexture destination)
		{
			...
			if (type == TonemapperType.UserCurve)
			{
				...
				tonemapMaterial.SetFloat(ShaderPropertyID._RangeScale, value);
				tonemapMaterial.SetTexture(ShaderPropertyID._Curve, curveTex);
				Graphics.Blit(source, destination, tonemapMaterial, 4);
				return;
			}
			if (type == TonemapperType.SimpleReinhard)
			{
				tonemapMaterial.SetFloat(ShaderPropertyID._ExposureAdjustment, exposureAdjustment);
			...
			{
				tonemapMaterial.SetFloat(ShaderPropertyID._ExposureAdjustment, 0.5f * exposureAdjustment);
			...
			tonemapMaterial.SetFloat(ShaderPropertyID._AdaptionSpeed, adaptionSpeed);
			...
			tonemapMaterial.SetTexture(ShaderPropertyID._SmallTex, rt);
