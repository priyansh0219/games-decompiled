...
namespace UnityStandardAssets.ImageEffects
{
	...
	public class VignetteAndChromaticAberration : PostEffectsBase
	{
		...
		{
			RenderEffect(src, dst, src.format);
		}

		public void RenderEffect(Texture source, RenderTexture destination, RenderTextureFormat format)
		{
			if (!CheckResources())
			{
			...
			if (flag)
			{
				...
				{
					renderTexture2 = RenderTexture.GetTemporary(width / 2, height / 2, 0, format);
					...
					for (int i = 0; i < 2; i++)
					{
						...
						RenderTexture temporary = RenderTexture.GetTemporary(width / 2, height / 2, 0, format);
