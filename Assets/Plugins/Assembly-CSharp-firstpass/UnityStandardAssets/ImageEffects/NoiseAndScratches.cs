...
namespace UnityStandardAssets.ImageEffects
{
	...
	public class NoiseAndScratches : MonoBehaviour
	{
		...
		private void OnRenderImage(RenderTexture source, RenderTexture destination)
		{
			...
			material.SetTexture(ShaderPropertyID._GrainTex, grainTexture);
			material.SetTexture(ShaderPropertyID._ScratchTex, scratchTexture);
			float num = 1f / grainSize;
			material.SetVector(ShaderPropertyID._GrainOffsetScale, new Vector4(Random.value, Random.value, (float)Screen.width / (float)grainTexture.width * num, (float)Screen.height / (float)grainTexture.height * num));
			material.SetVector(ShaderPropertyID._ScratchOffsetScale, new Vector4(scratchX + Random.value * scratchJitter, scratchY + Random.value * scratchJitter, (float)Screen.width / (float)scratchTexture.width, (float)Screen.height / (float)scratchTexture.height));
			material.SetVector(ShaderPropertyID._Intensity, new Vector4(Random.Range(grainIntensityMin, grainIntensityMax), Random.Range(scratchIntensityMin, scratchIntensityMax), 0f, 0f));
