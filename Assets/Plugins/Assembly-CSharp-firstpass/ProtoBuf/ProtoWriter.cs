	...
	public sealed class ProtoWriter : IDisposable
	{
		...
		private static void EndSubItem(SubItemToken token, ProtoWriter writer, PrefixStyle style)
		{
			...
			case PrefixStyle.Base128:
			{
				...
				if (num3 == 0)
				{
					writer.ioBuffer[num] = (byte)((uint)num2 & 0x7Fu);
					break;
				}
				...
				do
				{
					array[num++] = (byte)((num4 & 0x7Fu) | 0x80u);
				}
				while ((num4 >>= 7) != 0);
				array[num - 1] = (byte)(array[num - 1] & 0xFFFFFF7Fu);
		...
		{
			Init(dest, model, context);
		}

		public void Init(Stream dest, TypeModel model, SerializationContext context)
		{
			depth = 0;
			this.dest = null;
			fieldNumber = 0;
		...
		public static void WritePackedPrefix(int elementCount, WireType wireType, ProtoWriter writer)
		{
			...
			if (elementCount < 0)
			{
				throw new ArgumentOutOfRangeException(elementCount.ToString());
			}
			ulong value;
			switch (wireType)
			{
			...
			case WireType.Fixed64:
				...
				break;
			default:
				throw new ArgumentOutOfRangeException(wireType.ToString(), "Invalid wire-type: " + wireType);
			}
			WriteUInt64Variant(value, writer);
			writer.wireType = WireType.None;
		}
