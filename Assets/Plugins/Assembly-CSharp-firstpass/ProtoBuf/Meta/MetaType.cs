...
namespace ProtoBuf.Meta
{
	public class MetaType : ISerializerProxy
	{
		...
		internal void ApplyDefaultBehaviour()
		{
			...
			foreach (MemberInfo memberInfo in array3)
			{
				...
				if ((propertyInfo = memberInfo as PropertyInfo) != null)
				{
					...
					bool isField = false;
					ApplyDefaultBehaviour_AddMembers(model, attributeFamily, flag, basicList2, dataMemberOffset, flag2, implicitFields, basicList3, memberInfo, ref forced, isPublic, isField, ref effectiveType, backingMember);
				}
		...
		private static void ApplyDefaultBehaviour_AddMembers(TypeModel model, AttributeFamily family, bool isEnum, BasicList partialMembers, int dataMemberOffset, bool inferTagByName, ImplicitFields implicitMode, BasicList members, MemberInfo member, ref bool forced, bool isPublic, bool isField, ref Type effectiveType, MemberInfo backingMember = null)
		{
			...
			{
				ProtoMemberAttribute protoMemberAttribute = NormalizeProtoMember(model, member, family, forced, isEnum, partialMembers, dataMemberOffset, inferTagByName, backingMember);
