...
namespace ProtoBuf.Meta
{
	public abstract class TypeModel
	{
		...
		private static ObjectPool<ProtoWriter> writerPool = ObjectPoolHelper.CreatePool<ProtoWriter>("TypeModel::ProtoWriter", 32);
		...
		private object DeserializeWithLengthPrefix(Stream source, object value, Type type, PrefixStyle style, int expectedField, Serializer.TypeResolver resolver, out long bytesRead, out bool haveObject, SerializationContext context)
		{
			...
			try
			{
				...
				else if (!TryDeserializeAuxiliaryType(protoReader, DataFormat.Default, 1, type, ref value, skipOtherFields: true, asListItem: false, autoCreate: true, insideList: false) && num != 0L)
				{
					ThrowUnexpectedType(type);
				}
		...
		public void SerializeWithLengthPrefix(Stream dest, object value, Type type, PrefixStyle style, int fieldNumber, SerializationContext context)
		{
			...
			int key = GetKey(ref type);
			ProtoWriter protoWriter = writerPool.Get();
			protoWriter.Init(dest, this, context);
			switch (style)
			{
