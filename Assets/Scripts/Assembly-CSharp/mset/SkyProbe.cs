...
namespace mset
{
	public class SkyProbe
	{
		...
		public static void bindRandomValueTable(Material mat, string paramName, int inputFaceSize)
		{
			...
			mat.SetFloat(ShaderPropertyID._ImportantLog, f);
		}
		...
		public bool capture(Texture targetCube, Vector3 position, Quaternion rotation, bool HDR, bool linear, bool convolve)
		{
			...
			{
				camera.CopyFrom(MainCamera.camera);
			}
			camera.renderingPath = renderPath;
			camera.useOcclusionCulling = false;
		...
		private void copy_internal(Texture dstCube, Texture srcCube, bool dstRGBM, bool srcRGBM, bool linear, Camera cam, Material skyMat, Matrix4x4 matrix)
		{
			...
			toggleKeyword("MARMO_RGBM_INPUT_ON", srcRGBM);
			toggleKeyword("MARMO_RGBM_OUTPUT_ON", dstRGBM);
			skyMat.SetMatrix(ShaderPropertyID._SkyMatrix, matrix);
			skyMat.SetTexture(ShaderPropertyID._CubeHDR, srcCube);
		...
		private void convolve_internal(Texture dstTex, Texture srcCube, bool dstRGBM, bool srcRGBM, bool linear, Camera cam, Material skyMat, Matrix4x4 matrix)
		{
			...
			if ((bool)cubemap)
			{
				if (generateMipChain)
				{
					...
					{
						int width = 1 << num - i;
						skyMat.SetFloat(value: QPow.clampedDownShift(maxExponent, highestMipIsMirror ? (i - 1) : i, 1), nameID: ShaderPropertyID._SpecularExp);
						skyMat.SetFloat(ShaderPropertyID._SpecularScale, convolutionScale);
						Cubemap cubemap2 = new Cubemap(width, cubemap.format, mipChain: false);
				...
				{
					skyMat.SetFloat(ShaderPropertyID._SpecularExp, maxExponent);
					skyMat.SetFloat(ShaderPropertyID._SpecularScale, convolutionScale);
					cam.RenderToCubemap(cubemap);
				}
			}
