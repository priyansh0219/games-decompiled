...
namespace mset
{
	...
	public class SkyManager : MonoBehaviour
	{
		...
		private void HandleGlobalSkyChange()
		{
			if (nextSky != null)
			{
				...
				if (BlendingSupport && nextBlendTime > 0f)
				{
					...
					{
						array[i].ToggleChildLights(enable: false);
					}
		...
		public void SeekNewRenderers()
		{
			...
			{
				RegisterNewRenderer(array2[i]);
			}
			array = Object.FindObjectsOfType<SkinnedMeshRenderer>();
			array2 = array;
			for (int j = 0; j < array2.Length; j++)
			{
				RegisterNewRenderer(array2[j]);
			}
		}
		...
		private void EditorApplySkies(bool forceApply)
		{
			...
			object[] array = Object.FindObjectsOfType<MeshRenderer>();
			object[] renderers = array;
		...
		public void ProbeSkies(GameObject[] objects, Sky[] skies, bool probeAll, bool probeIBL)
		{
			...
			if (objects != null)
			{
				...
				{
					Sky component = objects[i].GetComponent<Sky>();
					if ((bool)component)
					{
			...
			if (list.Count > 0)
			{
				ProbeExposures = (probeIBL ? Vector4.one : Vector4.zero);
				SkiesToProbe = new Sky[list.Count];
				for (int j = 0; j < list.Count; j++)
				{
					SkiesToProbe[j] = list[j];
				}
			}
		}
	}
