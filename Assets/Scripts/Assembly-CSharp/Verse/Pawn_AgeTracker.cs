...
namespace Verse
{
	public class Pawn_AgeTracker : IExposable
	{
		...
		public void ResetAgeReversalDemand(AgeReversalReason reason, bool cancelInitialization = false)
		{
			...
			switch (reason)
			{
			case AgeReversalReason.Recruited:
				num = RecruitedPawnAgeReversalDemandInDays.RandomInRange;
				break;
			case AgeReversalReason.ViaTreatment:
				num = 60;
				break;
			default:
				num = NewPawnAgeReversalDemandInDays.RandomInRange;
				break;
			}
			long num2 = num * 60000;
			long num3 = Math.Max(AgeBiologicalTicks, 90000000L) + num2;
			if (reason != AgeReversalReason.Recruited || num3 >= ageReversalDemandedAtAgeTicks)
			{
				ageReversalDemandedAtAgeTicks = num3;
		...
		public void DebugSetAge(long finalAgeTicks)
		{
			...
			long NextBirthdayTick(long ageTicks)
			{
