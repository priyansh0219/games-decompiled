...
namespace UnityEngine.PostProcessing
{
	...
	public sealed class EyeAdaptationComponent : PostProcessingComponentRenderTexture<EyeAdaptationModel>
	{
		...
		public override bool active
		{
			get
			{
				if (base.model != null && base.model.enabled && SystemInfo.supportsComputeShaders)
				{
		...
		public Texture Prepare(RenderTexture source, Material uberMaterial)
		{
			...
			RenderTextureFormat format = ((source.format == RenderTextureFormat.DefaultHDR) ? PlatformUtils.defaultHDRFormat : source.format);
			RenderTexture renderTexture = context.renderTextureFactory.Get((int)histogramScaleOffsetRes.z, (int)histogramScaleOffsetRes.w, 0, format);
