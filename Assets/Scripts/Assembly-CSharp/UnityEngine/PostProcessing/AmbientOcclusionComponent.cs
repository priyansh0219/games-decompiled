...
namespace UnityEngine.PostProcessing
{
	public sealed class AmbientOcclusionComponent : PostProcessingComponentCommandBuffer<AmbientOcclusionModel>
	{
		...
		public override bool active
		{
			get
			{
				if (base.model != null && base.model.enabled && base.model.settings.intensity > 0f)
				{
		...
		public override void PopulateCommandBuffer(CommandBuffer cb)
		{
			...
			if (flag)
			{
				cb.GetTemporaryRT(occlusionTexture, width / num, height / num, 0, FilterMode.Bilinear, RenderTextureFormat.ARGB32, RenderTextureReadWrite.Linear, 1, enableRandomWrite: false, RenderTextureMemoryless.None, useDynamicScale: true);
			}
			else
			{
			...
			if (flag)
			{
				cb.GetTemporaryRT(occlusionTexture2, width, height, 0, FilterMode.Bilinear, RenderTextureFormat.ARGB32, RenderTextureReadWrite.Linear, 1, enableRandomWrite: false, RenderTextureMemoryless.None, useDynamicScale: true);
			}
			else
			{
			...
			if (flag)
			{
				cb.GetTemporaryRT(occlusionTexture, width, height, 0, FilterMode.Bilinear, RenderTextureFormat.ARGB32, RenderTextureReadWrite.Linear, 1, enableRandomWrite: false, RenderTextureMemoryless.None, useDynamicScale: true);
			}
			else
			{
			...
			else
			{
				...
				if (flag)
				{
					cb.GetTemporaryRT(tempRT, context.width, context.height, 0, FilterMode.Bilinear, format, RenderTextureReadWrite.Default, 1, enableRandomWrite: false, RenderTextureMemoryless.None, useDynamicScale: true);
				}
				else
				{
