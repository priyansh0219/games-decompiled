...
namespace UnityEngine.PostProcessing
{
	...
	public sealed class ScreenSpaceReflectionComponent : PostProcessingComponentCommandBuffer<ScreenSpaceReflectionModel>
	{
		...
		public override bool active
		{
			get
			{
				if (base.model != null && base.model.enabled && context.isGBufferAvailable)
				{
		...
		public override void PopulateCommandBuffer(CommandBuffer cb)
		{
			...
			if (flag)
			{
				cb.GetTemporaryRT(normalAndRoughnessTexture, -1, -1, 0, FilterMode.Point, RenderTextureFormat.ARGB32, RenderTextureReadWrite.Linear, 1, enableRandomWrite: false, RenderTextureMemoryless.None, useDynamicScale: true);
				cb.GetTemporaryRT(hitPointTexture, num2, num3, 0, FilterMode.Bilinear, RenderTextureFormat.ARGBHalf, RenderTextureReadWrite.Linear, 1, enableRandomWrite: false, RenderTextureMemoryless.None, useDynamicScale: true);
			}
			else
			{
			...
			for (int i = 0; i < 5; i++)
			{
				if (flag)
				{
					cb.GetTemporaryRT(m_ReflectionTextures[i], num2 >> i, num3 >> i, 0, FilterMode.Bilinear, format, RenderTextureReadWrite.Default, 1, enableRandomWrite: false, RenderTextureMemoryless.None, useDynamicScale: true);
				}
				else
				{
			...
			if (flag)
			{
				cb.GetTemporaryRT(filteredReflections, num2, num3, 0, (!k_BilateralUpsample) ? FilterMode.Bilinear : FilterMode.Point, format, RenderTextureReadWrite.Default, 1, enableRandomWrite: false, RenderTextureMemoryless.None, useDynamicScale: true);
				cb.GetTemporaryRT(finalReflectionTexture, num2, num3, 0, FilterMode.Point, format, RenderTextureReadWrite.Default, 1, enableRandomWrite: false, RenderTextureMemoryless.None, useDynamicScale: true);
			}
			else
			{
			...
			for (int j = 1; j < 5; j++)
			{
				...
				if (flag)
				{
					cb.GetTemporaryRT(blurTexture, num2 >> num9, num3 >> num9, 0, FilterMode.Bilinear, format, RenderTextureReadWrite.Default, 1, enableRandomWrite: false, RenderTextureMemoryless.None, useDynamicScale: true);
				}
				else
				{
			...
			if (flag)
			{
				cb.GetTemporaryRT(tempTexture, camera.pixelWidth, camera.pixelHeight, 0, FilterMode.Bilinear, format, RenderTextureReadWrite.Default, 1, enableRandomWrite: false, RenderTextureMemoryless.None, useDynamicScale: true);
			}
			else
			{
