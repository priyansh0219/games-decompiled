	...
	public sealed class MotionBlurComponent : PostProcessingComponentCommandBuffer<MotionBlurModel>
	{
		...
		public class ReconstructionFilter
		{
			...
			public void ProcessImage(PostProcessingContext context, CommandBuffer cb, ref MotionBlurModel.Settings settings, RenderTargetIdentifier source, RenderTargetIdentifier destination, Material material)
			{
				...
				bool num3 = DynamicResolution.IsEnabled();
				int velocityTex = Uniforms._VelocityTex;
				if (num3)
				{
					cb.GetTemporaryRT(velocityTex, context.width, context.height, 0, FilterMode.Point, m_PackedRTFormat, RenderTextureReadWrite.Linear, 1, enableRandomWrite: false, RenderTextureMemoryless.None, useDynamicScale: true);
				}
				else
				{
				...
				if (num3)
				{
					cb.GetTemporaryRT(tile2RT, context.width / 2, context.height / 2, 0, FilterMode.Point, m_VectorRTFormat, RenderTextureReadWrite.Linear, 1, enableRandomWrite: false, RenderTextureMemoryless.None, useDynamicScale: true);
				}
				else
				{
				...
				if (num3)
				{
					cb.GetTemporaryRT(tile4RT, context.width / 4, context.height / 4, 0, FilterMode.Point, m_VectorRTFormat, RenderTextureReadWrite.Linear, 1, enableRandomWrite: false, RenderTextureMemoryless.None, useDynamicScale: true);
				}
				else
				{
				...
				if (num3)
				{
					cb.GetTemporaryRT(tile8RT, context.width / 8, context.height / 8, 0, FilterMode.Point, m_VectorRTFormat, RenderTextureReadWrite.Linear, 1, enableRandomWrite: false, RenderTextureMemoryless.None, useDynamicScale: true);
				}
				else
				{
		...
		public class FrameBlendingFilter
		{
			private struct Frame
			{
				...
				public void MakeRecord(CommandBuffer cb, RenderTargetIdentifier source, int width, int height, Material material)
				{
					...
					{
						lumaTexture = RenderTexture.GetTemporary(width, height, 0, RenderTextureFormat.R8, RenderTextureReadWrite.Linear, 1, RenderTextureMemoryless.None, VRTextureUsage.None, useDynamicScale: true);
						chromaTexture = RenderTexture.GetTemporary(width, height, 0, RenderTextureFormat.R8, RenderTextureReadWrite.Linear, 1, RenderTextureMemoryless.None, VRTextureUsage.None, useDynamicScale: true);
					}
					else
					{
				...
				public void MakeRecordRaw(CommandBuffer cb, RenderTargetIdentifier source, int width, int height, RenderTextureFormat format)
				{
					...
					if (DynamicResolution.IsEnabled())
					{
						lumaTexture = RenderTexture.GetTemporary(width, height, 0, format, RenderTextureReadWrite.Default, 1, RenderTextureMemoryless.None, VRTextureUsage.None, useDynamicScale: true);
					}
					else
					{
		...
		public override void PopulateCommandBuffer(CommandBuffer cb)
		{
			...
			MotionBlurModel.Settings settings = base.model.settings;
			RenderTextureFormat format = (context.isHdr ? PlatformUtils.defaultHDRFormat : RenderTextureFormat.Default);
			int tempRT = Uniforms._TempRT;
			if (DynamicResolution.IsEnabled())
			{
				cb.GetTemporaryRT(tempRT, context.width, context.height, 0, FilterMode.Point, format, RenderTextureReadWrite.Default, 1, enableRandomWrite: false, RenderTextureMemoryless.None, useDynamicScale: true);
			}
			else
			{
