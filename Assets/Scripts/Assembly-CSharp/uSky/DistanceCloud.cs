...
namespace uSky
{
	...
	public class DistanceCloud : MonoBehaviour
	{
		...
		{
			float num = Mathf.Max(Mathf.Pow(0.25f, uSM.LinearSpace ? 1.5f : 1f), uSM.DayTime);
			num *= Mathf.Sqrt(uSM.Exposure);
			if (CloudMaterial != null)
			{
				CloudMaterial.SetVector(ShaderPropertyID.ShadeColorFromSun, uSM.LinearSpace ? (uSL.CurrentLightColor.linear * num) : (uSL.CurrentLightColor * num));
				CloudMaterial.SetVector(ShaderPropertyID.ShadeColorFromSky, uSM.LinearSpace ? (uSL.CurrentSkyColor.linear * num) : (uSL.CurrentSkyColor * num));
			}
		}
	}
}
