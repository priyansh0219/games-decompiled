...
public class Corpse : ThingWithComps, IThingHolder, IStrippable, IBillGiver, IObservedThoughtGiver
{
    ...
    public override void TickRare()
    {
        ...
        //Expel rot stink
        if (Spawned && this.GetRotStage() == RotStage.Rotting)
        {
            int rotStinkToProduce = GasUtility.RotStinkToSpawnForCorpse(this);
            if (rotStinkToProduce > 0)
                GasUtility.AddGas(Position, Map, GasType.RotStink, rotStinkToProduce);
        }
    ...
    public override IEnumerable<Gizmo> GetGizmos()
    {
        ...
        if (DebugSettings.ShowDevGizmos)
        {
            yield return new Command_Action()
            {
                ...
                action = () => ResurrectionUtility.Resurrect(InnerPawn)
            };
        }
    }
}
