...
public class Building_Door : Building
{
    ...
    private float OpenPct => Mathf.Clamp01((float)ticksSinceOpen / (float)TicksToOpenNow);	//Needs clamp for after game load	
	...
	public override void Draw()
	{
		...
		float offsetDist = VisualDoorOffsetStart + (VisualDoorOffsetEnd-VisualDoorOffsetStart) * OpenPct;	
		...
		for( int i=0; i<2; i++ )
		{
            ...
            Graphic.ShadowGraphic?.DrawWorker(doorPos, Rotation, def, this, 0);
        }
    ...
    private static int AlignQualityAgainst( IntVec3 c, IntVec3 offset, Map map )
    {
        ...
        for(int i=0; i<things.Count; i++ )
        {
            ...
            {
                var oppositeCell = c - offset;
                var oppositeDoor = oppositeCell.GetDoor(map);
                return oppositeDoor != null ? 5 : 1;
            }
            ...
            if( blue != null )
            {
                if( blue.def.entityDefToBuild.passability == Traversability.Impassable )
                    return 9;
                if( blue.def.entityDefToBuild is ThingDef thingDefToBuild && thingDefToBuild.IsFence )
                    return 1;
                if( typeof(Building_Door).IsAssignableFrom(t.def.thingClass) )
                    return 1;
            }
